...
class MedicalDeviceController extends AdminController
{
    /**
    ...
    {
        $grid = new Grid(new MedicalDevice());

        $grid->column('id', __('Id'));
        $grid->column('name', __('Name'));
        $grid->column('MedicalDeviceType.title', __('Category'));
    ...
    {
        $show = new Show(MedicalDevice::findOrFail($id));
    ...
    {
        $form = new Form(new MedicalDevice());
        $form->text('name', __('Name'));
        $form->select('type_id', __('Type_id'))->options((new MedicalDeviceType())::selectOptions());
